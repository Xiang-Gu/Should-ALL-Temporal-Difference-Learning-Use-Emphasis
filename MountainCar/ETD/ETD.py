...
def main():
    ...
    for idx in range(max_episodes):
        ...
        eligibility_trace = rho(state) * M * feature(state)  # Initialize eligibility trace e_0
        done = False
        while not done:
            ...
            eligibility_trace = rho(next_state) * (gamma * lam * eligibility_trace + M * feature(next_state))
