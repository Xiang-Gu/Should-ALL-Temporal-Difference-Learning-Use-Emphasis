...
import glob
import math
from plot_util import INF, compute_mean_and_SEM, get_all_files, get_mean_and_SEM_all_stepSizes, COMMON_PATH
...
def main():
    ...
    # [step_size, mean, SEM]
    result_TD = get_mean_and_SEM_all_stepSizes(TD_path, curve, performance_measure)
    result_ETD = get_mean_and_SEM_all_stepSizes(ETD_path, curve, performance_measure)
    ...
    plt.xlabel('Episodes')
    plt.ylabel('negated ' + str(performance_measure) + ' per episode\nAveraged over ' + str(len(measures_TD)) + ' runs', rotation=0)
