...
INF = 9999. # When performance measure is np.float('inf'), we replace it with this number so we can plot it to a graph
NUM_TRAINING_EPISODES = 3000 # Number of training episodes
COMMON_PATH = '/Users/rlai/Desktop/Research_UofA/PuddleWorld/'
...
def get_each_line(eachline, performance_measure):
    if performance_measure == 'step': result = float(eachline[0])
    elif performance_measure == 'reward': result = -float(eachline[1]) # negate reward (cost-to-go)
    else: assert False, 'performance measure argument should be either step or reward'

    if result == float('inf') or result == float('-inf'): # If one file goes to infinity, just assume this parameter setting will make the algorithm diverge
        result = INF
    return float(result)
...
def get_all_files(directory, performance_measure):
    ...
    for file_name in list_files:
        if not file_name.endswith('.err'):
            ...
            for eachline in fline:
                ...
                eachline = get_each_line(eachline, performance_measure)
...
def compute_mean_and_SEM(directory, curve, performance_measure):
    measures = get_all_files(directory, performance_measure)
    means_each_file = []
...
def get_mean_and_SEM_all_stepSizes(directory, curve, performance_measure):
    ...
    for each_step_size_directory in step_size_directories:
        ...
        mean, SEM = compute_mean_and_SEM(each_step_size_directory, curve, performance_measure)
