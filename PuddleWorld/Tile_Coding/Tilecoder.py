...
x_start, x_range = 0.0, 1.0
y_start, y_range = 0.0, 1.0
...
def feature(state, action):
    # Precondition check
    assert x_start <= state[0] <= (x_start + x_range) and y_start <= state[1] <= (y_start + y_range) and action in set(range(num_actions)), 'input out of space'
    ...
    for index in tileIndices:
            result[index + num_tiles_per_tiling * numTilings * action] += 1
    return result
...
def printTileCoderIndices(in1, in2, action):
    tileIndices = [-1] * numTilings
    tilecode(in1, in2, tileIndices)
    print('active tiles for input state (' + str(in1) + ', ' + str(in2) + ') is ' + str(tileIndices))
    print('feature for this input is: ' + str(feature([in1,in2], action)))


if __name__ == '__main__':
    printTileCoderIndices(0.0, 0.0, 0)
    printTileCoderIndices(0.0, 0.0, 1)
    printTileCoderIndices(0.0, 0.0, 2)
    printTileCoderIndices(0.5, 0.3, 1)
    printTileCoderIndices(0.25, 0.01, 2)
