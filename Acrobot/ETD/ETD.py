...
'''Use ETD(lambda) to do Control on Acrobot'''
...
def main():
    ...
    for idx in range(max_episodes):
        ...
        for idx in feature(state, action):
            eligibility_trace[idx] += rho(state) * M
        done = False
        ...
        while not done:
            ...
            for idx in feature(next_state, next_action):
                eligibility_trace[idx] += rho(next_state) * M
